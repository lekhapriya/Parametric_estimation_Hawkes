...
void Basic::initializeParams(ArrayXXd &arr)
{
    //params.setRandom(no_of_params, no_of_nodes); //setRandom [-1,1]
    arr = 0+(ArrayXXd::Random(no_of_nodes, no_of_nodes)*0.5+0.5)*(1-0); //[0,1] [low high]
    //cout<<params<<endl;
    return;
}

void Basic::initializeBaseline(ArrayXd &arr)
{
     arr = 0+(ArrayXd::Random(no_of_nodes)*0.5+0.5)*(1-0); //[0,1] [low high]
}
